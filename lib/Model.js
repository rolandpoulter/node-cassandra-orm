...
module.exports = require('clss')('Model', function (def) {
    ...
    var self = this;
		...
		function execute(connection) {
			connection.execute(command, callback, self || this);
	...
	this.automigrate = function (callback, connection) {
		...
		return connection.execute('select * from system.schema_columnfamilies where keyspace_name=\'' + keyspace + '\' and columnfamily_name=\'' + name + '\'', function (err, rows) {
		  if (err) {
        return that.connection.keyspace.get(name, function (err, columnFamily) {
          delete that.connection.keyspace.columnFamilies[name]; // Don't cache me
          ...
          if (columnFamily) {
            ...
            for (var i in columnFamily.columnValidators) {
              ...
              var col = {name:i, type: type, index:cm.index_type || cm.index_name};
      ...
      if (same)
		...
		function createIndexes (err) {
			...
			async.forEach(that.schema.getIndexes(), function (name, next) {
			  ...
			  if (err) return callback.bind(that)(err);
			  if (orm.schemaSync) {
			    that.connection = connection;
			    orm.schemaSync(that, callback.bind(that));
			  } else
