        ...
        return connection.keyspace.get(name, function (err, columnFamily) {
          ...
          if (columnFamily) {
            ...
            for (var i in columnFamily.columnValidators) {
              ...
              var hasNonSOLRIndex = cm.index_type && (cm.index_options.class_name !== 'com.datastax.bdp.cassandra.index.solr.SolrSecondaryIndex');
              var col = {name:i, type: type, index:hasNonSOLRIndex};
    ...
    function matchColumns(columns) {
      ...
      Object.keys(that.schema.info.fields).forEach(function (name) {
        ...
        if (!/^_.*/.test(name) && !columns[name] && name !== 'id') {
          same = false;
          var type = that.schema.info.fields[name].type || 'Object';
      ...
      if (same)
		...
		function createIndexes (err) {
			...
			if (false && connection.options.enableDse) {
			  if (err) {
			    that.migrating = false;
