...
module.exports = require('clss')('Model', function (def) {
  ...
  this.provideId = function (keys, cb) {
    var k = [];
    var n = [];
    keys = keys || {};
    var valid = true;
    ...
    this.schema.info.primaryKeys.forEach(function (key, i) {
      k.push(keys[key]);
      n.push(self.schema.info.fields[key]);
      if (i !== (self.schema.info.primaryKeys.length - 1) && (keys[key] === undefined))
        valid = false;
    });
    if (!valid)
      return cb(new Error('Invalid key content ' + JSON.stringify(keys)));
    var last = n.length - 1;
    ...
    case 'timestamp':
      forceEncoding = true;
      if (!k[last])
        k[last] = new Date().getTime();
      break;
    case 'timeuuid':
      if (!k[last])
        k[last] = new TimeUUID().toString();
      break;
    case 'uuid':
      if (!k[last])
        k[last] = new UUID().toString();
      break;
    default:
      return cb(new Error('Invalid column type for automatic key generation'));
    }
//    console.log(k);
    if (k.length === 1 && !forceEncoding)
      return cb(null, k[0]);
    return cb(null, ':' + new Buffer(JSON.stringify(k)).toString('base64'));
  }
  ...
  this.automigrate = function (callback, connection) {
    ...
    return connection.execute('select * from system.schema_columnfamilies where keyspace_name=\'' + keyspace + '\' and columnfamily_name=\'' + name + '\'', function (err, rows) {
      ...
      if (rows.length > 0) {
        var column_aliases = rows[0].get('column_aliases').value ? JSON.parse(rows[0].get('column_aliases').value) : null;
        var key_aliases = rows[0].get('key_aliases').value ? JSON.parse(rows[0].get('key_aliases').value) : null;
        var key_alias = key_aliases || [rows[0].get('key_alias').value];
        var key_validator = rows[0].get('key_validator').value.replace("org.apache.cassandra.db.marshal.CompositeType(", "").replace(")", "").split(',').map(function (e) {
          return e.replace(/\t */g, '');
        });
        var comp = rows[0].get('comparator').value.replace("org.apache.cassandra.db.marshal.CompositeType(", "").replace(")", "").split(',').map(function (e) {
          return e.replace(/\t */g, '');
        });
//        console.log(comparator);
        var comparator = [];
        comp.forEach(function (e) {
          var match = /org.apache.cassandra.db.marshal.ReversedType\(([^\)]*)\)/.exec(e);
          if (match && match[1]) {
            comparator.push({name:match[1],reversed:true});
          } else {
            comparator.push({name:e,reversed:false});
          }
        });
        return connection.execute('select * from system.schema_columns where keyspace_name=\'' + connection.keyspace.name + '\' and columnfamily_name=\'' + name + '\'', function (err, rows) {
          var columns = {};
          var index = 0;
          column_aliases.forEach(function (key) {
            var col = {};
            col.name = key;
            col.index = false;
            col.reversed = comparator[index].reversed;
            col.isColumnKey = true;
            switch (comparator[index++].name) {
            case 'org.apache.cassandra.db.marshal.BytesType':
              col.type = 'blob';
              break;
            case 'org.apache.cassandra.db.marshal.BooleanType':
            ...
            case 'org.apache.cassandra.db.marshal.Int32Type':
              ...
              break;
            default:
              console.log('COLUMNS Unknown type: "', comparator[index - 1], '"');
              col.type = 'Object';
              break;
            }
            columns[col.name] = col;
          });
          key_aliases.forEach(function (key, i) {
            var col = {};
            col.name = key;
            col.index = false;
            col.isColumnKey = false;
            var match = /org.apache.cassandra.db.marshal.ReversedType\(([^\)]*)\)/.exec(key_validator[i]);
            if (match && match[1]) {
              col.reversed = true;
              key_validator = match[1];
            }
            switch (key_validator[i]) {
            case 'org.apache.cassandra.db.marshal.BytesType':
              col.type = 'blob';
              break;
            case 'org.apache.cassandra.db.marshal.BooleanType':
            ...
            case 'org.apache.cassandra.db.marshal.Int32Type':
              ...
              break;
            default:
              console.log('KEYS Unknown type: "', key_validator, '"');
              col.type = 'Object';
              break;
            }
            columns[col.name] = col;
          ...
          rows.forEach(function (row) {
            ...
            case 'org.apache.cassandra.db.marshal.Int32Type':
              ...
              break;
            default:
              console.log('ROWS Unknown type: "', validator, '"');
              col.type = 'Object';
              break;
            }
            columns[col.name] = col;
...
//      console.log(that.cql.tableName(), columns);
      ...
      Object.keys(columns).forEach(function (name) {
        var d = that.schema.info.fields[name];
        var s = columns[name];
        d.index |= s.index;
        d.reversed = s.reversed;
        d.isColumnKey = s.isColumnKey;
      });
