...
module.exports = require('clss').create('CQLWriter', function (def) {
  ...
  def.update = function (data, conds, options) {
    ...
    function setField(name, i) {
      var isKey = (name === 'id') || (self.schema.info.fields[name] && self.schema.info.fields[name].primary_key);
      if (!isKey && !self.schema.info.fields[name])
        console.log('Field not in schema ' + name);
      if (isKey)
        return null;
  ...
  def.propertiesSQL = function (fields, keys) {
    ...
    Object.keys(this.tableFields()).forEach(function (name) {
      ...
      if (keys.indexOf(name) === -1) {
        console.log(name, 'is not in keys', keys);
        cql.push(this.propertySQL(name, fields));
      }
    }.bind(this));

    if (keys.length > 1) cql.push('PRIMARY KEY (' + keys.map(this.escapeName.bind(this)).join(', ') + ')');
