...
module.exports = require('clss').create('CQLWriter', function (def) {
	...
	def.selectOne = function (conds, order, offset, consistency) {
		return this.selectAll({where: conds, limit: 1, order: order, offset: offset}, consistency);
	};
	...
	def.update = function (data, conds, options) {
		...
		return ' SET ' + sets.join(', ') + this.where(conds) + ';';
	...
	def.truncate = function (name, keyspace) {
		return 'TRUNCATE ' + (keyspace ? this.escapeName(keyspace) + '.' : '') + this.escapeName(name) + ';';
	};
	...
	def.alterTable = function (changes, name, keyspace) {
		...
		if (changes.drops) changes.alters.forEach(function (drop) {
			statements.push(alterTable + ' DROP ' + this.escapeName(drop));
		}.bind(this));

		if (changes.adds) changes.alters.forEach(function (add) {
	...
	def.where = function (conds) {
		...
		Object.keys(conds).forEach(function (name) {
			if (!conds[name]) return;
	
			var condition = conds[name],
			    value = this.schema.toDatabase(condition, name);

			name = this.escapeName(name);

			if (Array.isArray()) cql.push(name + ' IN (' + value + ')');
